...
def high_population_gdps
  ...
  execute(<<-SQL)
  select name, gdp/population from countries where population >= 200000000
  SQL
end
...
def population_in_millions
  ...
  # millions.
  execute(<<-SQL)
  select name, population/1000000 from countries where continent = 'South America'
  SQL
end
...
def name_and_population
  ...
  execute(<<-SQL)
  select name, population from countries where name in ('France', 'Germany', 'Italy')
  SQL
end
