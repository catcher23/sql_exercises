...
def films_from_sixty_two
  ...
  execute(<<-SQL)
  select id, title from movies where yr = 1962
  SQL
end
...
def trek_films
  ...
  # year.
  execute(<<-SQL)
  select id, title, yr from movies where title like 'Star Trek%' order by yr
  SQL
end
...
def casablanca_cast
  ...
  join movies on movies.id = castings.movie_id where movies.id =
  (select id from movies where title = 'Casablanca')
  SQL
end
...
def alien_cast
  ...
  execute(<<-SQL)
  select actors.name from actors join castings on actors.id = castings.actor_id
  join movies on movies.id = castings.movie_id where movies.title = 'Alien'
  SQL
end
