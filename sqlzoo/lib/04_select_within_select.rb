...
def richer_than_england
  ...
  # 'United Kingdom'.
  execute(<<-SQL)
  select name from countries where gdp/population > (select gdp/population from countries where name = 'United Kingdom')
...
def population_constraint
  ...
  execute(<<-SQL)
  select name, population from countries where population between (select population from countries where name = 'Canada') + 1
  and (select population from countries where name = 'Poland') - 1
  SQL
end
...
def sparse_continents
  ...
  execute(<<-SQL)
  select distinct name, continent, population from countries where continent not in
