...
def group_population
  ...
  execute(<<-SQL)
  select sum(population) from countries where name in ('France', 'Germany', 'Spain')
  SQL
end
...
def populous_continents
  ...
  execute(<<-SQL)
  select continent from countries group by continent having sum(population) > 100000000
  SQL
end
